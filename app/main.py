...
async def _sync_xray_state(session_maker, xray_service: XrayService) -> None:
    ...
    payload = [(user.telegram_id, str(user.uuid)) for user in users]
    await xray_service.sync_enabled_users(payload)
...
async def main() -> None:
    ...
    payment_service = YooMoneyService(settings, session_maker)
