...
Активный пользователь получает в одной Happ-подписке несколько отдельных элементов основного VPN: `Основной #1 🇳🇱` для Reality Vision `89.125.50.96:443/tcp`, `Основной #2 🇳🇱` для Reality no-flow `89.125.50.96:8443/tcp`, `Основной #3 🇳🇱` для XHTTP/TLS `s2.nnqnn.tech:8444/kvpn-xhttp` и `Основной #4 🇳🇱` для Hysteria2 `89.125.50.96:443/udp`. Если куплен обход белых списков, профиль worker-а `vpn.nnqnn.tech` добавляется пятым отдельным элементом с названием `Обход белых списков`. На server #2 публичный TCP `443` слушает Xray напрямую; Nginx stream на `443` не используется. HTTPS-подписка доступна отдельно на `s2.nnqnn.tech:8444`. Reality `upstream-in:9443` остается включенным для старой цепочки `server #1 -> server #2`.
